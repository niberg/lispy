...
enum { LVAL_ERR, LVAL_NUM, LVAL_DOUBLE, LVAL_SYM, LVAL_STR, 
...
int main(int argc, char** argv) 
{
  ...
  mpca_lang(MPCA_LANG_DEFAULT,
      ...
      lispy   : /^/ <expr>* /$/ ;                  \
      ",
      Double, Number, Symbol, String, Comment, Sexpr, Qexpr, Expr, Lispy);

  lenv* e = lenv_new();
  lenv_add_builtins(e);

  /* Load standard library */
  char filename[] = {"prelude.lispy"};
  lval* prelude = lval_add(lval_sexpr(), lval_str(filename));
  lval* x = builtin_load(e, prelude);
  if (x->type == LVAL_ERR)
    lval_println(x);
  lval_del(x);
  ...
  mpc_cleanup(9, 
      Double, Number, Symbol, String, Comment, 
